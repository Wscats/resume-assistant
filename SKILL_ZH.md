...
> AI 驱动的 clawbot 技能，提供简历润色、职位定制、多格式导出和专业评分一站式服务。
...
简历助手（Resume Assistant）是一个 clawbot 技能，帮助求职者创建、优化和完善简历。它整合了全面的检查清单审查、专业评分、职位匹配分析和多格式导出功能，提供从简历撰写到投递的完整工作流。
...
Resume Assistant 是一个标准的 clawbot skill，可以被任何兼容的 AI Agent 加载和调用。以下是在不同场景下的接入方式。

### 方式一：通过 clawbot 平台直接加载

在 clawbot 对话中，直接输入斜杠命令即可使用：
